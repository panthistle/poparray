...
# ---- PATH/PROFILE EDIT OPS DRAW-FUNCTION
...
def polygon(cns, cvs, pgob, isprof=False):
    ...
    row = cvs.row(align=True)
    rc = row.column(align=True)
    rc.prop(pgob, "pol_sid", text="")
    rc = row.column(align=True)
    rc.prop(pgob, "pol_ang", text="")
    rc = row.column(align=True)
    rc.enabled = pgob.pol_sid == 4
    rc.prop(pgob, "pol_xrs", text="")
    row = cvs.row(align=True)
    rc = row.column(align=True)
...
# ---- PATH/PROF/OB_LOC-ROT, OBSCA EDIT OPS PARAMS DRAW-FUNCTION
