...
def aniact_path_edit_dict(path, npts, loop):
    d = {}
    adim = path.ani_dim
    ...
    idlst = aniact_index_offset_list(path.ani_nidx, path.pathed.idx, loop)
    d["idx"] = [i % npts for i in idlst]
    p_n = path.provider
    if p_n == "custom":
...
def aniact_prof_edit_dict(prof, npts, loop):
    d = {}
    adim = prof.ani_dim
    ...
    idlst = aniact_index_offset_list(prof.ani_idx, prof.profed.idx, loop)
    d["idx"] = [i % npts for i in idlst]
    p_n = prof.provider
    if p_n == "custom":
...
def aniact_collang_list(inst, loop):
    ...
    if not (inst.ani_rot.active and angle):
        return [0] * loop
    f1 = inst.ani_rot.beg
    f2 = inst.ani_rot.end
...
def aniact_edrots_dict(coll, loop):
    d = {"angs": [], "use_facs": [], "lids": []}
    tmpid = 0
    for item in coll:
        if item.active:
            if item.anim_state():
