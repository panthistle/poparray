...
class PTDBLNPOPA_vec3(bpy.types.PropertyGroup):
...
class PTDBLNPOPA_anim_amount(bpy.types.PropertyGroup):
    active: bpy.props.BoolProperty(
...
class PTDBLNPOPA_batchcoll_toggle(bpy.types.PropertyGroup):
    ...
    object: bpy.props.BoolProperty(
        name="object",
        description="toggle object edits",
        default=False,
        options={"HIDDEN"},
    )


class PTDBLNPOPA_batchcoll_update(bpy.types.PropertyGroup):
    edits: bpy.props.PointerProperty(type=PTDBLNPOPA_batchcoll_toggle)
...
class PTDBLNPOPA_params(bpy.types.PropertyGroup):
...
class PTDBLNPOPA_noiz(bpy.types.PropertyGroup):
    def noiz_active_update(self, context):
        pool = context.scene.ptdblnpopa_pool
        if pool.update_ok:
            bpy.ops.ptdblnpopa.pop_simple_update()
...
class PTDBLNPOPA_pathloc(bpy.types.PropertyGroup):
...
class PTDBLNPOPA_pathrot(bpy.types.PropertyGroup):
...
class PTDBLNPOPA_profloc(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(default="Location")
    active: bpy.props.BoolProperty(default=False)
    fac: bpy.props.FloatProperty(default=0)
    axis: bpy.props.FloatVectorProperty(size=3, default=(1, 1, 1))
...
class PTDBLNPOPA_profrot(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(default="Rotation")
    active: bpy.props.BoolProperty(default=False)
...
class PTDBLNPOPA_pathed(bpy.types.PropertyGroup):
    ...
    upv: bpy.props.CollectionProperty(type=PTDBLNPOPA_vec3)
...
class PTDBLNPOPA_path(bpy.types.PropertyGroup):
    def path_provider_update(self, context):
        pool = context.scene.ptdblnpopa_pool
        if pool.update_ok:
            bpy.ops.ptdblnpopa.setup_provider(caller="path")

    def path_res_update(self, context):
        pool = context.scene.ptdblnpopa_pool
        if pool.update_ok:
            bpy.ops.ptdblnpopa.update_replace()
    ...
    pathed: bpy.props.PointerProperty(type=PTDBLNPOPA_pathed)
    ani_nidx: bpy.props.PointerProperty(type=PTDBLNPOPA_anim_index)
    ani_dim: bpy.props.BoolProperty(
        name="size", description="animate dimensions", default=False, options={"HIDDEN"}
    )
    ani_dim_mirror: bpy.props.PointerProperty(type=PTDBLNPOPA_anim_mirror)
    ...
    def anim_state(self):
        if self.ani_nidx.active or self.ani_dim:
            return True
...
class PTDBLNPOPA_profed(bpy.types.PropertyGroup):
    ...
    def profed_common_update(self, context):
        pool = context.scene.ptdblnpopa_pool
        if pool.update_ok:
            bpy.ops.ptdblnpopa.pop_simple_update()
...
class PTDBLNPOPA_prof(bpy.types.PropertyGroup):
    def prof_provider_update(self, context):
        pool = context.scene.ptdblnpopa_pool
        if pool.update_ok:
            bpy.ops.ptdblnpopa.setup_provider(caller="prof")
    ...
    profed: bpy.props.PointerProperty(type=PTDBLNPOPA_profed)
    ani_idx: bpy.props.PointerProperty(type=PTDBLNPOPA_anim_index)
    ani_dim: bpy.props.BoolProperty(
        name="size", description="animate dimensions", default=False, options={"HIDDEN"}
    )
    ...
    def anim_state(self):
        if self.ani_idx.active or self.ani_dim:
            return True
...
class PTDBLNPOPA_obloc(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(default="Location")
    active: bpy.props.BoolProperty(default=False)
    fac: bpy.props.FloatProperty(default=0)
    axis: bpy.props.FloatVectorProperty(size=3, default=(1, 1, 1))
...
class PTDBLNPOPA_obrot(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(default="Rotation")
    active: bpy.props.BoolProperty(default=False)
    axis: bpy.props.FloatVectorProperty(size=3, default=(0, 0, 1))
    angle: bpy.props.FloatProperty(default=0, subtype="ANGLE")
...
class PTDBLNPOPA_obsca(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(default="Scale")
    active: bpy.props.BoolProperty(default=False)
    fac: bpy.props.FloatProperty(default=0)
    axis: bpy.props.FloatVectorProperty(size=3, default=(1, 1, 1))
    nprams: bpy.props.PointerProperty(type=PTDBLNPOPA_params)
    ...
    def to_dct(self):
        d = {"fac": self.fac, "axis": self.axis}
        d["nprams"] = self.nprams.to_dct()
        d["iprams"] = self.iprams.to_dct()
        return d


class PTDBLNPOPA_attitude(bpy.types.PropertyGroup):
...
class PTDBLNPOPA_rngs(bpy.types.PropertyGroup):
...
class PTDBLNPOPA_track_actions(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(default="name")


class PTDBLNPOPA_track(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(default="Track")
    idns: bpy.props.CollectionProperty(type=PTDBLNPOPA_track_actions)
...
class PTDBLNPOPA_pool(bpy.types.PropertyGroup):
    def pool_full_att_update(self, context):
        if self.update_ok:
            bpy.ops.ptdblnpopa.pop_simple_update()

    path: bpy.props.PointerProperty(type=PTDBLNPOPA_path)
    path_att: bpy.props.PointerProperty(type=PTDBLNPOPA_attitude)
    custom_path_use_edits: bpy.props.BoolProperty(
        name="use active edits",
        description="include active location/rotation edits",
        default=False,
        options={"HIDDEN"},
    )
    prof: bpy.props.PointerProperty(type=PTDBLNPOPA_prof)
    prof_att: bpy.props.PointerProperty(type=PTDBLNPOPA_attitude)
    full_att: bpy.props.BoolProperty(
        name="path-cluster attitude",
        description=(
            "rotation from tangent vectors and direction: will use the "
            "path track-axis and optionally the path up-axis"
        ),
        default=False,
        update=pool_full_att_update,
        options={"HIDDEN"},
    )
    noiz: bpy.props.PointerProperty(type=PTDBLNPOPA_noiz)
    rngs: bpy.props.PointerProperty(type=PTDBLNPOPA_rngs)
    pathrot: bpy.props.CollectionProperty(type=PTDBLNPOPA_pathrot)
    pathrot_idx: bpy.props.IntProperty(name="Rot", default=-1, options={"HIDDEN"})
    pathloc: bpy.props.CollectionProperty(type=PTDBLNPOPA_pathloc)
    pathloc_idx: bpy.props.IntProperty(name="Loc", default=-1, options={"HIDDEN"})
    profrot: bpy.props.CollectionProperty(type=PTDBLNPOPA_profrot)
    profrot_idx: bpy.props.IntProperty(name="Rot", default=-1, options={"HIDDEN"})
    profloc: bpy.props.CollectionProperty(type=PTDBLNPOPA_profloc)
    profloc_idx: bpy.props.IntProperty(name="Loc", default=-1, options={"HIDDEN"})
    obrot: bpy.props.CollectionProperty(type=PTDBLNPOPA_obrot)
    obrot_idx: bpy.props.IntProperty(name="Rot", default=-1, options={"HIDDEN"})
    obloc: bpy.props.CollectionProperty(type=PTDBLNPOPA_obloc)
    obloc_idx: bpy.props.IntProperty(name="Loc", default=-1, options={"HIDDEN"})
    obsca: bpy.props.CollectionProperty(type=PTDBLNPOPA_obsca)
    obsca_idx: bpy.props.IntProperty(name="Sca", default=-1, options={"HIDDEN"})
    trax: bpy.props.CollectionProperty(type=PTDBLNPOPA_track)
    trax_idx: bpy.props.IntProperty(name="Track", default=-1, options={"HIDDEN"})
    OBJ_SUPPORT = {"EMPTY", "MESH", "CURVE", "FONT", "LIGHT"}
    batchtoggle_ops: bpy.props.PointerProperty(type=PTDBLNPOPA_batchcoll_toggle)
    batchupdate_ops: bpy.props.PointerProperty(type=PTDBLNPOPA_batchcoll_update)

    def pool_sample_ob_check(self, object):
        if object.type not in self.OBJ_SUPPORT:
            return False
        if self.setcoll and self.replace_set and (object.name in self.setcoll.objects):
            return False
        return True

    def pool_source_update(self, context):
        if self.update_ok:
            bpy.ops.ptdblnpopa.update_sample()
    ...
    sample_ob: bpy.props.PointerProperty(
        name="source object",
    ...
    setcoll_name: bpy.props.StringProperty(default="apop_set")
    replace_set: bpy.props.BoolProperty(
        name="replace set",
        default=False,
        description=(
            "if enabled, selecting a new 'source object' or running any of the "
            "'Load ...' commands will replace current set"
        ),
        options={"HIDDEN"},
    )
    ...
    anicalc: bpy.props.PointerProperty(type=PTDBLNPOPA_anicalc)
    ...
    act_loc: bpy.props.BoolProperty(
        name="location",
        description="animate location",
        default=False,
        options={"HIDDEN"},
    )
    act_rot: bpy.props.BoolProperty(
        name="rotation",
        description="animate rotation",
        default=False,
        options={"HIDDEN"},
    )
    act_sca: bpy.props.BoolProperty(
        name="scale", description="animate scale", default=False, options={"HIDDEN"}
    )
    shqu: bpy.props.BoolProperty(
        name="short rotation path",
        description=(
            "force rotation quaternions to take the short path between keyframes"
        ),
        default=False,
        options={"HIDDEN"},
    )

    def pop_anim_state_data_eval(self):
    ...
    def obj_anim_state_scas_eval(self):
        prof_on = self.use_profile
        for item in self.obsca:
            if item.active and item.anim_state(prof_on):
                return True
        return False
    ...
    def to_dct(self):
        return {
            "defsca": list(self.sample_ob.scale),
            "pathori": self.path_att.active,
            "pathori_track": self.path_att.track,
            "pathori_up": self.path_att.up,
            "pathori_upfixed": self.path_att.upfixed,
            "profori": self.prof_att.active,
            "profori_track": self.prof_att.track,
            "profori_up": self.prof_att.up,
            "profori_upfixed": self.prof_att.upfixed,
            "fullori": self.full_att,
        }
...
class PTDBLNPOPA_OT_path_edit(bpy.types.Operator):
    bl_label = "Path Settings"
    bl_idname = "ptdblnpopa.path_edit"
    ...
    def invoke(self, context, event):
        path = context.scene.ptdblnpopa_pool.path
...
class PTDBLNPOPA_OT_prof_edit(bpy.types.Operator):
    bl_label = "Profile Settings"
    bl_idname = "ptdblnpopa.prof_edit"
    ...
    def invoke(self, context, event):
        prof = context.scene.ptdblnpopa_pool.prof
        self.provider = prof.provider
        d = prof.profed.to_dct(exclude={"upv", "idx", "closed"})
        for key in d.keys():
    ...
    def execute(self, context):
        ...
        pool.update_ok = False
        prof = pool.prof
        d = self.profed.to_dct(exclude={"npts", "upv", "idx", "closed"})
        for key in d.keys():
...
class PTDBLNPOPA_OT_pathloc_edit(bpy.types.Operator):
    bl_label = "Path Locations"
    bl_idname = "ptdblnpopa.pathloc_edit"
...
class PTDBLNPOPA_OT_pathrot_edit(bpy.types.Operator):
    bl_label = "Path Rotations"
    bl_idname = "ptdblnpopa.pathrot_edit"
    ...
    brots: bpy.props.EnumProperty(
        name="rotation effect",
        description="apply rotation to path locations, rotations or both",
        items=(
            ("locs", "locations", "path locations"),
            ("rots", "rotations", "path rotations"),
            ("both", "both", "both rotations and locations"),
        ),
        default="both",
    )
    ...
    def draw(self, context):
        ...
        row = sc.row(align=True)
        row.prop(self, "brots", text="")
        row = sc.row(align=True)
        row.enabled = self.brots in {"locs", "both"}
...
class PTDBLNPOPA_OT_profloc_edit(bpy.types.Operator):
    bl_label = "Profile Locations"
    bl_idname = "ptdblnpopa.profloc_edit"
    bl_description = "profile locations"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}

    profatt_active: bpy.props.BoolProperty(default=False)
    fac: bpy.props.FloatProperty(name="factor", description="amount", default=0)
    axis: bpy.props.FloatVectorProperty(
        name="influence",
        description="axis factor",
    ...
    bbatt: bpy.props.BoolProperty(
        name="update attitude",
        description="effective when profile attitude is enabled",
        default=True,
    )
    ...
    def invoke(self, context, event):
        ...
        self.profatt_active = pool.prof_att.active
        self.fac = item.fac
        self.axis = item.axis
        self.abs_move = item.abs_move
        self.bbatt = item.bbatt
    ...
    def draw(self, context):
        ...
        row.prop(self, "fac", text="")
        row = sc.row(align=True)
        row.enabled = self.profatt_active
        row.prop(self, "bbatt", toggle=True)
...
class PTDBLNPOPA_OT_profrot_edit(bpy.types.Operator):
    bl_label = "Profile Rotations"
    bl_idname = "ptdblnpopa.profrot_edit"
    ...
    bbatt: bpy.props.EnumProperty(
        ...
        description="rotation sequence",
        items=(
            ("after", "after attitude", "effective when profile attitude is enabled"),
            ("before", "before attitude", "effective when profile attitude is enabled"),
        ),
        default="after",
    )
    brots: bpy.props.EnumProperty(
        name="rotation effect",
        description="apply rotation to profile locations, rotations or both",
        items=(
            ("locs", "locations", "profile locations"),
            ("rots", "rotations", "profile rotations"),
            ("both", "both", "both rotations and locations"),
        ),
        default="both",
    )
...
# ---- OBJECT LOCATIONS COLLECTION ITEM EDITOR


class PTDBLNPOPA_OT_obloc_edit(bpy.types.Operator):
    bl_label = "Object Locations"
    bl_idname = "ptdblnpopa.obloc_edit"
    bl_description = "object locations"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}

    use_profile: bpy.props.BoolProperty(default=False)
    abs_move: bpy.props.BoolProperty(
        name="move", description="global (world) or local (object) axis", default=False
    )
    axis: bpy.props.FloatVectorProperty(
        name="influence",
        description="axis factor",
    ...
    def execute(self, context):
        ...
        try:
            ...
            pool.update_ok = True
            print(f"obloc_edit: {my_err.args}")
            self.report({"INFO"}, f"{my_err.args}")
            return {"CANCELLED"}
        pool.update_ok = True
        return {"FINISHED"}
    ...
    def draw(self, context):
        ...
        row.prop(
            self,
            "abs_move",
            text=("global axis" if self.abs_move else "local axis"),
            toggle=True,
        )
        row = sc.row(align=True)
        row.prop(self, "axis", text="")
...
# ---- OBJECT ROTATIONS COLLECTION ITEM EDITOR


class PTDBLNPOPA_OT_obrot_edit(bpy.types.Operator):
    bl_label = "Object Rotations"
    bl_idname = "ptdblnpopa.obrot_edit"
    bl_description = "object rotations"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            pool.update_ok = True
            print(f"obrot_edit: {my_err.args}")
            self.report({"INFO"}, f"{my_err.args}")
            return {"CANCELLED"}
        pool.update_ok = True
        return {"FINISHED"}
...
# ---- OBJECT SCALE COLLECTION ITEM EDITOR


class PTDBLNPOPA_OT_obsca_edit(bpy.types.Operator):
    bl_label = "Object Scales"
    bl_idname = "ptdblnpopa.obsca_edit"
    bl_description = "object scales"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            pool.update_ok = True
            print(f"obsca_edit: {my_err.args}")
            self.report({"INFO"}, f"{my_err.args}")
            return {"CANCELLED"}
        pool.update_ok = True
        return {"FINISHED"}
...
classes = (
    ...
    PTDBLNPOPA_pathed,
    PTDBLNPOPA_profed,
    PTDBLNPOPA_track_actions,
    PTDBLNPOPA_batchcoll_toggle,
    PTDBLNPOPA_batchcoll_update,
    PTDBLNPOPA_noiz,
    PTDBLNPOPA_pathrot,
    PTDBLNPOPA_pathloc,
    PTDBLNPOPA_path,
    ...
    PTDBLNPOPA_anicalc,
    PTDBLNPOPA_pool,
    PTDBLNPOPA_OT_path_edit,
    PTDBLNPOPA_OT_pathloc_edit,
    PTDBLNPOPA_OT_pathrot_edit,
    PTDBLNPOPA_OT_prof_edit,
    PTDBLNPOPA_OT_profloc_edit,
    PTDBLNPOPA_OT_profrot_edit,
    PTDBLNPOPA_OT_obloc_edit,
    PTDBLNPOPA_OT_obrot_edit,
    PTDBLNPOPA_OT_obsca_edit,
)
...
def register():
    ...
    for cls in classes:
        register_class(cls)
    bpy.types.Scene.ptdblnpopa_params = bpy.props.PointerProperty(
        type=PTDBLNPOPA_params
    )
    bpy.types.Scene.ptdblnpopa_pathed = bpy.props.PointerProperty(
        type=PTDBLNPOPA_pathed
    )
    bpy.types.Scene.ptdblnpopa_profed = bpy.props.PointerProperty(
        type=PTDBLNPOPA_profed
    )
    bpy.types.Scene.ptdblnpopa_pool = bpy.props.PointerProperty(type=PTDBLNPOPA_pool)
...
def unregister():
    ...
    del bpy.types.Scene.ptdblnpopa_pool
    del bpy.types.Scene.ptdblnpopa_profed
    del bpy.types.Scene.ptdblnpopa_pathed
    del bpy.types.Scene.ptdblnpopa_params
