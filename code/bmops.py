...
class PTDBLNPOPA_OT_custom_path_create(bpy.types.Operator):
    bl_label = "Custom Path Object"
    bl_idname = "ptdblnpopa.custom_path_create"
    bl_description = "create a mesh object from current path settings"
    ...
    def execute(self, context):
        ...
        try:
            ModPOPA.create_path_object(scene)
        except Exception as my_err:
            pool.update_ok = True
            print(f"custom_path_create: {my_err.args}")
            return {"CANCELLED"}
        pool.update_ok = True
        return {"FINISHED"}


class PTDBLNPOPA_OT_pop_simple_update(bpy.types.Operator):
    bl_label = "Simple Update"
    bl_idname = "ptdblnpopa.pop_simple_update"
...
class PTDBLNPOPA_OT_update_sample(bpy.types.Operator):
    bl_label = "Update Sample"
    bl_idname = "ptdblnpopa.update_sample"
    ...
    def execute(self, context):
        ...
        pool.update_ok = False
        try:
            ModFNOP.validate_scene_object(scene, pool.sample_ob)
            if not pool.sample_ob:
                raise Exception("missing source object!")
            pool.trax.clear()
            pool.trax_idx = -1
            pool.animorph = False
            ModPOPA.scene_update_newset(scene, replace=pool.replace_set)
        except Exception as my_err:
            pool.update_ok = True
            print(f"update_sample: {my_err.args}")
            return {"CANCELLED"}
        pool.update_ok = True
        return {"FINISHED"}


class PTDBLNPOPA_OT_pop_reset(bpy.types.Operator):
    bl_label = "Update Settings"
    bl_idname = "ptdblnpopa.pop_reset"
    ...
    def execute(self, context):
        ...
        try:
            ...
            pool.trax_idx = -1
            pool.animorph = False
            replace = pool.replace_set
            if self.newdef:
                pool.props_unset()
            ModPOPA.scene_update_newset(scene, replace=replace, rename_coll=self.newdef)
        except Exception as my_err:
            pool.update_ok = True
...
class PTDBLNPOPA_OT_poparray_setup(bpy.types.Operator):
    bl_label = "Pop Array Setup"
    bl_idname = "ptdblnpopa.poparray_setup"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}
    ...
    def description(cls, context, properties):
        if getattr(properties, "single"):
            return f"active interface: 2D Array"
        return f"active interface: Single Array"
    ...
    def execute(self, context):
        ...
        try:
            ...
            ModPOPA.scene_update_newset(scene, replace=True)
        except Exception as my_err:
            pool.update_ok = True
            print(f"poparray_setup: {my_err.args}")
            self.report({"INFO"}, f"{my_err.args}")
            return {"CANCELLED"}
        pool.update_ok = True
        return {"FINISHED"}
...
class PTDBLNPOPA_OT_obrange_react(bpy.types.Operator):
    bl_label = "Range Active Toggle"
    bl_idname = "ptdblnpopa.obrange_react"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            pool.update_ok = True
            print(f"obrange_react: {my_err.args}")
            self.report({"INFO"}, f"{my_err.args}")
            return {"CANCELLED"}
        pool.update_ok = True
        return {"FINISHED"}
...
class PTDBLNPOPA_OT_batchcoll_toggle(bpy.types.Operator):
...
class PTDBLNPOPA_OT_setup_provider(bpy.types.Operator):
    bl_label = "Provider Update"
    bl_idname = "ptdblnpopa.setup_provider"
...
class PTDBLNPOPA_OT_update_replace(bpy.types.Operator):
    bl_label = "Update Replace"
    bl_idname = "ptdblnpopa.update_replace"
    bl_description = "pop setup"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}
...
class PTDBLNPOPA_OT_pop_noiz(bpy.types.Operator):
    bl_label = "Noise Locations"
    bl_idname = "ptdblnpopa.pop_noiz"
    ...
    def poll(cls, context):
        return context.scene.ptdblnpopa_pool.noiz.active

    def invoke(self, context, event):
        noiz = context.scene.ptdblnpopa_pool.noiz
...
# ---- PATHLOC/PATHROT/PROFLOC/OBLOC/OBROT/OBSCA COLLECTIONS OPERATORS


class PTDBLNPOPA_OT_citem_add(bpy.types.Operator):
    bl_label = "Add"
    bl_idname = "ptdblnpopa.citem_add"
    ...
    def execute(self, context):
        ...
        try:
            ...
            if cname not in {"pathloc", "pathrot"}:
...
class PTDBLNPOPA_OT_citem_copy(bpy.types.Operator):
    bl_label = "Copy"
    bl_idname = "ptdblnpopa.citem_copy"
...
class PTDBLNPOPA_OT_citem_enable(bpy.types.Operator):
    bl_label = "Enable"
    bl_idname = "ptdblnpopa.citem_enable"
...
class PTDBLNPOPA_OT_citem_remove(bpy.types.Operator):
    bl_label = "Remove"
    bl_idname = "ptdblnpopa.citem_remove"
...
class PTDBLNPOPA_OT_citem_move(bpy.types.Operator):
    bl_label = "Stack"
    bl_idname = "ptdblnpopa.citem_move"
...
class PTDBLNPOPA_OT_write_setts(bpy.types.Operator, ExportHelper):
    bl_label = "Save Settings"
    bl_idname = "ptdblnpopa.write_setts"
...
class PTDBLNPOPA_OT_read_setts(bpy.types.Operator, ImportHelper):
    bl_label = "Load Settings"
    bl_idname = "ptdblnpopa.read_setts"
    ...
    def execute(self, context):
        ...
        try:
            ...
            ModPOPA.scene_update_newset(scene, replace=replace, rename_coll=True)
        except Exception as my_err:
            pool.update_ok = True
...
class PTDBLNPOPA_OT_animorph_setup(bpy.types.Operator):
    bl_label = "Exit Animation Mode?"
    bl_idname = "ptdblnpopa.animorph_setup"
    bl_options = {"REGISTER", "INTERNAL", "UNDO"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            if pool.trax:
                print("---- poparray animode: delete trash")
...
class PTDBLNPOPA_OT_anicycmirend(bpy.types.Operator):
    bl_label = "Set Loop Range"
    bl_idname = "ptdblnpopa.anicycmirend"
...
class PTDBLNPOPA_OT_anicalc(bpy.types.Operator):
    bl_label = "Anicalc"
    bl_idname = "ptdblnpopa.anicalc"
    ...
    def execute(self, context):
        scene = context.scene
        clc = scene.ptdblnpopa_pool.anicalc
...
class PTDBLNPOPA_OT_track_edit(bpy.types.Operator):
    bl_label = "Track Settings"
    bl_idname = "ptdblnpopa.track_edit"
    ...
    def execute(self, context):
        ...
        try:
            ...
            for ob, idn in zip(oblst, item.idns):
                tsn = idn.name
                strip = ob.animation_data.nla_tracks[tsn].strips[tsn]
...
class PTDBLNPOPA_OT_track_enable(bpy.types.Operator):
    bl_label = "Enable"
    bl_idname = "ptdblnpopa.track_enable"
    ...
    def execute(self, context):
        ...
        try:
            ...
            if self.doall:
                ...
                for ob, idn in zip(oblst, item.idns):
                    ob.animation_data.nla_tracks[idn.name].mute = boo_mute
                item.active = not boo_mute
        except Exception as my_err:
            pool.update_ok = True
...
class PTDBLNPOPA_OT_track_remove(bpy.types.Operator):
    bl_label = "Remove"
    bl_idname = "ptdblnpopa.track_remove"
    ...
    def execute(self, context):
        ...
        try:
            ...
            if self.doall:
                ...
                item = pool.trax[idx]
                for ob, idn in zip(oblst, item.idns):
                    t = ob.animation_data.nla_tracks.get(idn.name)
                    if t:
                        ob.animation_data.nla_tracks.remove(t)
                pool.trax.remove(idx)
                pool.trax_idx = min(max(0, idx - 1), len(pool.trax) - 1)
            print("---- poparray track_remove: delete trash")
            bpy.ops.outliner.orphans_purge(do_recursive=True)
        except Exception as my_err:
            pool.update_ok = True
...
class PTDBLNPOPA_OT_track_copy(bpy.types.Operator):
    bl_label = "Copy"
    bl_idname = "ptdblnpopa.track_copy"
...
class PTDBLNPOPA_OT_anim_action(bpy.types.Operator):
    bl_label = "Animation"
    bl_idname = "ptdblnpopa.anim_action"
    ...
    def execute(self, context):
        ...
        rings = pool.path.pathed.npts
        use_profile = pool.use_profile
        short_quat = pool.shqu
        rpts = pool.prof.profed.npts if use_profile else 1
        items = rings * rpts
        loop = pool.ani_kf_loop
        ...
        try:
            ...
            action_location = pool.act_loc and pop_ani_dat
            action_rotation = pool.act_rot and pop_ani_dat
            action_scale = pool.act_sca and pool.obj_anim_state_scas_eval()
            if not (action_location or action_rotation or action_scale):
        ...
        ksca = []
        try:
            pop = ModPOPA.new_pop_instance(pool)
            ModPOPA.pop_update(pop, pool)
            for i in range(loop):
                if pop_ani_dat:
                    if path_flag:
                    ...
                    if action_rotation:
                        if sindz_on:
                            krot.append([rots[j] for j in range(items) if j in sindz])
                        else:
                            krot.append(rots)
                    if action_location:
                        if noiz.active:
                            locs = ModPOPA.noiz_locs(
                                locs, noiz.vfac, noiz_d["ampli"][i], noiz_d["seed"]
                            )
                        if sindz_on:
                            kloc.append([locs[j] for j in range(items) if j in sindz])
                        else:
                            kloc.append(locs)
                if action_scale:
                    if use_profile:
        ...
        try:
            ...
            for i, ob in enumerate(oblst):
                ...
                if action_location:
                    dp = "location"
                    for di in range(3):
                        vls = [v[i][di] for v in kloc]
                        fc_create(action, dp, di, fls, vls, kls, loop)
                if action_rotation:
                    dp = "rotation_quaternion"
                    for di in range(4):
                        vls = [q[i][di] for q in krot]
                        fc_create(action, dp, di, fls, vls, kls, loop)
                if action_scale:
                    dp = "scale"
                    for di in range(3):
                        vls = [v[i][di] for v in ksca]
                        fc_create(action, dp, di, fls, vls, kls, loop)
                tn = trk.idns.add()
                tn.name = action.name
                ModFNOP.aniact_nla_track_add(ob, action)
            k_end = fls[-1]
            trk.ac_beg = k_beg
            trk.ac_end = k_end
            trk.sa_beg = k_beg
...
classes = (
    PTDBLNPOPA_OT_custom_path_create,
    PTDBLNPOPA_OT_pop_simple_update,
    PTDBLNPOPA_OT_update_sample,
    PTDBLNPOPA_OT_pop_reset,
    PTDBLNPOPA_OT_poparray_setup,
    PTDBLNPOPA_OT_obrange_react,
    PTDBLNPOPA_OT_batchcoll_toggle,
    PTDBLNPOPA_OT_batchcoll_update,
    PTDBLNPOPA_OT_setup_provider,
    PTDBLNPOPA_OT_update_replace,
    PTDBLNPOPA_OT_pop_noiz,
    PTDBLNPOPA_OT_citem_add,
    PTDBLNPOPA_OT_citem_copy,
    PTDBLNPOPA_OT_citem_enable,
    PTDBLNPOPA_OT_citem_remove,
    PTDBLNPOPA_OT_citem_move,
    PTDBLNPOPA_OT_write_setts,
    PTDBLNPOPA_OT_read_setts,
    PTDBLNPOPA_OT_animorph_setup,
    PTDBLNPOPA_OT_anicycmirend,
    PTDBLNPOPA_OT_anicalc,
    PTDBLNPOPA_OT_track_edit,
    PTDBLNPOPA_OT_track_enable,
    PTDBLNPOPA_OT_track_remove,
    PTDBLNPOPA_OT_track_copy,
    PTDBLNPOPA_OT_anim_action,
)
